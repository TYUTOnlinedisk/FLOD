...
class ClearTrimMeanClient(WorkerBase):
    def __init__(self, client_id, model, loss_func, train_iter, test_iter, config, optimizer, grad_stub):
        super(ClearTrimMeanClient, self).__init__(model=model, loss_func=loss_func, train_iter=train_iter,
...
if __name__ == '__main__':
    ...
    with grpc.insecure_channel(server_grad, options=config.grpc_options) as grad_channel:
        ...
        client = ClearTrimMeanClient(client_id=args.id, model=model, loss_func=loss_func, train_iter=train_iter,
                                  test_iter=test_iter, config=config, optimizer=optimizer, grad_stub=grad_stub)

        client.fl_train(times=args.E)
        client.write_acc_record(fpath="Eva/clear_trimmean_acc.txt", info="clear_trimmean_acc_worker")
